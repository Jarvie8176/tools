...
def serve(port: int, host: str = "127.0.0.1", refresh: int = 3) -> None:
    ...
    # These all-interfaces sentinels are compared to WARN the operator, not to bind to them (the
    # bind default is 127.0.0.1). B104 flags the literal regardless, so suppress it on this line.
    if host in ("0.0.0.0", "::", ""):  # noqa: S104  # nosec B104
