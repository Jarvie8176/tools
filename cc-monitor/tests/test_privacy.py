...
def _render_row(**kw):
    base = {"status": "active", "ctx": 0, "win": 200000, "win_certain": True, "cum_input": 0,
...
def test_html_redacts_prompt_and_title_when_enabled():
    ...
    assert "abcd1234" in out and "active" in out  # structural fields stay visible
...
def test_serialize_redacts_free_text_but_not_structure(monkeypatch):
    ...
    row = {"session_id": "s", "u8": "u", "pid": 1, "name": "n", "model": "m", "status": "active",
    ...
    assert s["u8"] == "u" and s["status"] == "active" and s["ctx"] == 10  # structure intact
