...
_cache: dict = {"key": None, "value": None}  # (path, mtime) -> merged config
...
def load(path: str | None = None) -> dict:
    ...
    key = (path, mtime)
    if _cache["key"] == key:
        return dict(_cache["value"])  # copy — callers must not mutate the cache
    cfg = dict(DEFAULTS)
    raw = _read_raw(path)
    for k in SCHEMA:
        if k in raw:
    ...
    _cache["key"], _cache["value"] = key, cfg
    return dict(cfg)
